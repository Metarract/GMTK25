...
func _init(bn:String, d:String, t:Texture2D, wgt:float, ms:float, btv:float, col:Color=Color.TRANSPARENT, a:float=-1, c:float=-1, h:float=-1, j:float=-1, l:int=-1, s:float=-1) -> void:
	bug_name = bn
	description = d
	texture = t
	weight = wgt
